...
```
```{r}
espn_raw <- read_csv("nfl_draft_prospects.csv")
espn.tot <-espn_raw %>% filter(!is.na(espn_raw$grade))
espn.qb <- espn.tot %>% filter(pos_abbr== "QB")
write.csv(espn.qb, file = "espn.qb.csv")
```
